                ...
                <div class="col-5">
                    ...
                    @if(count($langs) != 1)
                      ...
                      <select class="form-select" id="lang-select" name="lang-select" onchange="window.location.href=this.options[this.selectedIndex].value;">
                          @foreach ($langs as $lang)
                          @if ($lang->is_active)
                          <option value="{{ route('admin.panel-maintenance.index',['lang_code'=>$lang->lang_code, 'page'=>request('page')]) }}"
                            @if ($lang_code==$lang->lang_code)
                                selected
                            @endif
      ...
      <div class="card-body">
        ...
        <div class="tab-content" id="top-tabContent">
          <div class="tab-pane fade @if (!session()->get('tab_page') || session()->get('tab_page') == 'page_content')) show active @endif" id="top-page-content" role="tabpanel" aria-labelledby="top-page-content-tab">
            <form class="theme-form needs-validation" novalidate="" action="{{ route('admin.panel-maintenance.update',[$content->id]) }}" method="POST">
              @csrf
              @method('PUT')
                ...
                <div class="row">
                  ...
                  <div class="col-md-6 col-lg-4 col-xl-4 col-xxl-3">
                    <div class="input-group" >
                      <input style="width:100%;" type="submit" class="btn btn btn-primary my-0 mx-1" id="updatepanelmaintenance" name="updatepanelmaintenance" value="{{__('admin_app_maintenance.update')}}">
                    </div>
                  </div>
                </div>
              </div>
          ...
          <div class="tab-pane fade @if (session()->get('tab_page') == 'page_image_large') show active @endif" id="top-page-image-large" role="tabpanel" aria-labelledby="top-page-image-large">
            <form action="{{ route('admin.panel-maintenance.update',[$content->id]) }}" method="POST" enctype="multipart/form-data">
              @csrf
              @method('PUT')
              <input type="hidden" name="page" value="{{ request('page') }}">
              <input type="hidden" name="lang_code" value="{{ request('lang_code') }}">
