...
@section('title',__('admin_products_page.update_content'))
...
<div class="page-body pt-5">
    ...
    <div class="page-header">
      <div class="row">
        <div class="col-7">
            <div class="row">
                <div class="col-8">
                    <h3>{{__('admin_products_page.update_content')}} @if(count($langs) != 1)<span class="title-lang-code">({{ request('lang_code') }})</span>@endif</h3>
                    <ol class="breadcrumb">
                      ...
                      <li class="breadcrumb-item disappear-500">{{__('admin_products_page.update_content')}}</li>
                    </ol>
                </div>
                <div class="col-4">
                    <!--
                    ...
                    @if(count($langs) != 1)
                      @foreach ($langs as $lang)
                        ...
                        <div class="mx-2 d-none">
                          <span class="disabled">({{__('admin_products_page.current_language')}})</span>
                      ...
                      <select class="form-select" id="lang-select" name="lang-select" onchange="window.location.href=this.options[this.selectedIndex].value;">
                          @foreach ($langs as $lang)
                          @if ($lang->is_active)
                          <option value="{{ route('admin.products-info-settings.index',['lang_code'=>$lang->lang_code, 'page'=>request('page')]) }}"
                            @if ($lang_code==$lang->lang_code)
                                selected
                            @endif
        ...
        <div class="col-5 d-flex justify-content-end">
          ...
          <a href="{{ route('slug',$content->slug) }}" target="_blank">
            <button type="button" class="btn btn-primary btn-sm m-1">
              <i class="fas fa-external-link-alt me-1"></i> <span> {{__('admin_products_page.website')}}</span>
            </button>
          </a>
        </div>
      </div>
  ...
  <!-- Sweet Alert -->
  @if(session('success'))
  <script>swal("{{__('admin_products_page.success')}}", "{{__('admin_products_page.update_success_context')}}", "success");</script>
  @elseif(session('error'))
  <script>swal("{{__('admin_products_page.error')}}", "{{__('admin_products_page.update_error_context')}}", "error");</script>
  @elseif(session('file_extension_error'))
  <script>swal("{{__('admin_products_page.error')}}", "{{__('admin_products_page.update_error_file_mime')}}", "error");</script>
  @elseif(session('file_size_error'))
  <script>swal("{{__('admin_products_page.error')}}", "{{__('admin_products_page.update_error_file_size')}}", "error");</script>
  @endif
      ...
      <div class="card-body">
        <ul class="nav nav-tabs border-tab" id="top-tab" role="tablist">
          <li class="nav-item"><a class="nav-link @if (!session('tab_page') || session()->get('tab_page') == 'context')) active @endif" data-bs-toggle="tab"
              ...
              <i class="fas fa-book-open"></i>{{__('admin_products_page.content')}}</a></li>
          <li class="nav-item"><a class="nav-link @if (session()->get('tab_page') == 'image_large') active @endif" data-bs-toggle="tab"
              ...
              <i class="fas fa-desktop"></i>{{__('admin_products_page.big_image')}}</a></li>
          <li class="nav-item"><a class="nav-link @if (session()->get('tab_page') == 'image_small') active @endif" data-bs-toggle="tab"
              ...
              <i class="fas fa-mobile-alt"></i>{{__('admin_products_page.small_image')}}</a></li>
          <li class="nav-item"><a class="nav-link @if (session()->get('tab_page') == 'image_banner') active @endif" data-bs-toggle="tab"
              ...
              <i class="fa-solid fa-panorama"></i>{{__('admin_products_page.banner_image')}}</a></li>
        </ul>
        <div class="tab-content" id="top-tabContent">
          <div class="tab-pane fade @if (!session()->get('tab_page') || session()->get('tab_page') == 'context')) show active @endif" id="top-content" role="tabpanel" aria-labelledby="top-content-tab">
            <form class="theme-form needs-validation" novalidate="" action="{{ route('admin.products-info-settings.update',[$content->id]) }}" method="POST">
              @csrf
              @method('PUT')
              <input type="hidden" name="page" value="{{ request('page') }}">
              <h4 class="border-bottom pb-2">{{__('admin_products_page.productpage_content')}}</h4>
              <div class="mb-3">
                <label class="col-form-label p-0" for="title">{{__('admin_products_page.title')}}</label><span class="text-danger">*</span>
                <input class="form-control" id="title" name="title" type="text" placeholder="{{__('admin_products_page.title_placeholder')}}" value="{{ $content->title }}" required="">
              ...
              <div class="mb-3">
                ...
                <input class="form-control" id="subtitle" name="subtitle" type="text" placeholder="{{__('admin_products_page.subtitle_placeholder')}}" value="{{ $content->subtitle }}">
              </div>
              <div class="mb-3">
                <label class="col-form-label p-0" for="menu_name">{{__('admin_products_page.menu_name')}}</label><span class="text-danger">*</span>
                <input class="form-control" id="menu_name" name="menu_name" type="text" placeholder="{{__('admin_products_page.menu_name_placeholder')}}" value="{{ $content->menu_name }}" required="">
                <div class="valid-feedback">{{__('admin_products_page.looks_good')}}</div>
              </div>
              <div class="mb-3">
                <label class="col-form-label p-0" for="page_limit">{{__('admin_products_page.page_product_limit')}}</label>
                <input class="form-control" id="page_limit" name="page_limit" type="number" placeholder="{{__('admin_products_page.page_product_limit_placeholder')}}" value="{{ $content->page_limit }}">
              </div>
              <div class="mb-3">
                <label class="col-form-label p-0" for="video_embed_code">{{__('admin_products_page.youtube_code')}}</label>
                <input class="form-control" id="video_embed_code" name="video_embed_code" type="text" placeholder="{{__('admin_products_page.youtube_code_placeholder')}}" value="{{ $content->video_embed_code }}">
              </div>
              <div class="mb-4">
                <label class="form-label p-0" for="keywords">{{__('admin_products_page.keywords')}}</label>
                <textarea class="form-control" id="keywords" name="keywords" rows="4" placeholder="{{__('admin_products_page.keywords_placeholder')}}">{{ $content->keywords }}</textarea>
              </div>
              <div class="mb-4">
                <label class="form-label p-0" for="description">{{__('admin_products_page.description')}}</label>
                <textarea class="form-control" id="description" name="description" rows="8" placeholder="{{__('admin_products_page.description_placeholder')}}">{{ $content->description }}</textarea>
              </div>
              <div class="mb-4">
                <label class="form-label p-0" for="short_description">{{__('admin_products_page.short_description')}}</label>
                <textarea class="form-control" id="short_description" name="short_description" rows="6" placeholder="{{__('admin_products_page.short_description_placeholder')}}">{{ $content->short_description }}</textarea>
              </div>
              <div>
                <div class="form-check form-switch my-0">
                  ...
                  <label class="form-check-label custom-switch-label" for="is_desktop_active">{{__('admin_products_page.large_screen_active')}}</label>
                </div>
              </div>
              <div class="mb-4">
                <div class="form-check form-switch my-0">
                ...
                <div class="row">
                  ...
                  <div class="col-md-6 col-lg-4 col-xl-4 col-xxl-3">
                    <div class="input-group" >
                      <input style="width:100%;" type="submit" class="btn btn btn-primary my-0 mx-1" id="updateproductsinfo" name="updateproductsinfo" value="{{__('admin_products_page.update')}}">
                    </div>
                  </div>
                </div>
              </div>
          ...
          <div class="tab-pane fade @if (session()->get('tab_page') == 'image_large') show active @endif" id="top-image-large" role="tabpanel" aria-labelledby="profile-top-tab">
            <form action="{{ route('admin.products-info-settings.update',[$content->id]) }}" method="POST" enctype="multipart/form-data">
              @csrf
              @method('PUT')
              <input type="hidden" name="page" value="{{ request('page') }}">
              <!-- Image Cropper -->
              <div class="row items-push d-flex align-items-stretch">
                <div class="col-xxl-6">
                  <h4 class="border-bottom pb-2">{{__('admin_products_page.large_screen_image')}}</h4>
                  <!--
                  ...
                  <div id="box" class="block-content text-center d-none cropper-buttons">
                    <div class="row">
                      ...
                      data-method="crop" title="{{__('admin_products_page.crop_popup')}}">
                      {{__('admin_products_page.crop')}}
                      </button>
                    </div>
                  </div>
                  <div class="my-4">
                    <input type="file" class="form-control" id="image" name="image_large_screen" accept="image/*">
                    <small class="fw-light text-gray-dark">{{__('admin_products_page.file_mimes')}}</small>
                    <input type="hidden" name="cropped_data" id="cropped_data"> <!--base64 data transferi-->
                  </div>
                </div>

                <div id="box-2" class="col-xxl-6 d-none">
                  <h4 class="border-bottom pb-2">{{__('admin_products_page.preview')}}</h4>
                  <!--rightbox-->
                  ...
                  <div class="row">
                    <input type="submit" class="btn btn-lg btn-primary updateimagebutton" id="updateimagelarge" name="updateimagelarge" value="{{__('admin_products_page.upload_image')}}">
                  </div>
                </div>
              </div>
              <!-- END Image Cropper -->
          ...
          <div class="tab-pane fade @if (session()->get('tab_page') == 'image_small') show active @endif" id="top-image-small" role="tabpanel" aria-labelledby="contact-top-tab">
            <form action="{{ route('admin.products-info-settings.update',[$content->id]) }}" method="POST" enctype="multipart/form-data">
              ...
              <div class="row items-push d-flex align-items-stretch">
                <div class="col-xxl-6">
                  <h4 class="border-bottom pb-2">{{__('admin_products_page.small_screen_image')}}</h4>
                  <!--
                    RATIO & WIDTH Ayarları.
                  -->
                ...
                <div id="box-2-mobile" class="col-xxl-6 d-none">
                  ...
                  <div class="row">
                    <input type="submit" class="btn btn-lg btn-primary updateimagebutton" id="updateimagesmall" name="updateimagesmall" value="{{__('admin_products_page.upload_image')}}">
                  </div>
                </div>
              </div>
              <!-- END Image Cropper -->
          ...
          <div class="tab-pane fade @if (session()->get('tab_page') == 'image_banner') show active @endif" id="top-image-banner" role="tabpanel" aria-labelledby="top-image-banner-tab">
            <form action="{{ route('admin.products-info-settings.update',[$content->id]) }}" method="POST" enctype="multipart/form-data">
              ...
              <div class="row items-push d-flex align-items-stretch">
                <div class="col-xxl-6">
                  <h4 class="border-bottom pb-2">{{__('admin_products_page.banner_image')}}</h4>
                  <!--
                    RATIO & WIDTH Ayarları.
                  -->
                ...
                <div id="box-2-banner" class="col-xxl-6 d-none">
                  ...
                  <div class="row">
                    <input type="submit" class="btn btn-lg btn-primary updateimagebutton" id="updateimagebanner" name="updateimagebanner" value="{{__('admin_products_page.upload_image')}}">
                  </div>
                </div>
              </div>
              <!-- END Image Cropper -->
