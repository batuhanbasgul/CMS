...
@section('title',__('admin_slider.update_slider_order'))
...
@section('content')
  ...
  <div class="page-body pt-5">
      ...
      <div class="page-header">
        <div class="row">
          <div class="col-7">
            <div class="row">
                <div class="col-7">
                    <h3>{{__('admin_slider.order_sliders')}} @if(count($langs) != 1)<span class="title-lang-code">({{ request('lang_code') }})</span>@endif</h3>
                    <ol class="breadcrumb">
                      ...
                      <li class="breadcrumb-item disappear-500">{{__('admin_slider.order_sliders')}}</li>
                    </ol>
                </div>
                <div class="col-5">
                    <!--
                    ...
                    @if(count($langs) != 1)
                      @foreach ($langs as $lang)
                        ...
                        <div class="mx-2 d-none">
                          <span class="disabled">({{__('admin_slider.current_language')}})</span>
                      ...
                      <select class="form-select" id="lang-select" name="lang-select" onchange="window.location.href=this.options[this.selectedIndex].value;">
                          @foreach ($langs as $lang)
                          @if ($lang->is_active)
                          <option value="{{ route('admin.slider-settings.index',['lang_code'=>$lang->lang_code, 'page'=>request('page')]) }}"
                            @if (request('lang_code')==$lang->lang_code)
                                selected
                            @endif
    ...
    <div class="container-fluid">
      <div class="card">
        <div class="card-body">
          <form class="js-validation" action="{{ route('admin.slider-settings.update',[$sliders[0]->id]) }}" method="POST" enctype="multipart/form-data">
            @csrf
            @method('PUT')
                  ...
                  <div class="item" style="background-image: url({{ asset('admin-asset/gray-scale.png') }}), url({{ asset($item->image_large_screen) }});">
                    <div class="d-flex row m-1">
                      ...
                      <div class="col-2 px-2 ms-3">
                        <a class="btn btn-sm btn-primary px-2" href="{{ route('admin.slider-settings.edit', [$item->id]) }}">
                          <i class="fa fa-pencil-alt"></i>
                        </a>
                      </div>
                      <div class="col-2" onclick="return confirm('{{__('admin_slider.delete_question')}}')">
                        <div class="col-2">
                          <a class="btn btn-sm btn-danger px-2" href="{{ route('admin.slider-settings.destroy', $item->id) }}">
              ...
              <div class="row">
                ...
                <div class="col-md-6 col-lg-4 col-xl-4 col-xxl-3">
                  <div class="input-group" >
                    <small class="font-weight-bold ms-2">{{__('admin_slider.drag_to_order')}}</small>
                    <input style="width:100%;" type="submit" class="btn btn btn-primary my-2 mx-1" id="updateslidersorder" name="updateslidersorder" value="{{__('admin_slider.save_order')}}">
                  </div>
                </div>
              </div>
            </div>
