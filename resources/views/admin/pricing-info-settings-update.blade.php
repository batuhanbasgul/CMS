...
@section('title',__('admin_pricing_page.update_content'))
...
<div class="page-body pt-5">
    ...
    <div class="page-header">
      <div class="row">
        <div class="col-7">
            <div class="row">
                <div class="col-8">
                    <h3>{{__('admin_pricing_page.update_content')}} @if(count($langs) != 1)<span class="title-lang-code">({{ request('lang_code') }})</span>@endif</h3>
                    <ol class="breadcrumb">
                      ...
                      <li class="breadcrumb-item disappear-500">{{__('admin_pricing_page.update_content')}}</li>
                    </ol>
                </div>
                <div class="col-4">
                    <!--
                    ...
                    @if(count($langs) != 1)
                      @foreach ($langs as $lang)
                        ...
                        <div class="mx-2 d-none">
                          <span class="disabled">({{__('admin_pricing_page.current_language')}})</span>
                      ...
                      <select class="form-select" id="lang-select" name="lang-select" onchange="window.location.href=this.options[this.selectedIndex].value;">
                          @foreach ($langs as $lang)
                          @if ($lang->is_active)
                          <option value="{{ route('admin.pricing-info-settings.index',['lang_code'=>$lang->lang_code, 'page'=>request('page')]) }}"
                            @if ($lang_code==$lang->lang_code)
                                selected
                            @endif
        ...
        <div class="col-5 d-flex justify-content-end">
            <!--
              To pricing page content gallery.
            -->
          <a href="{{ route('admin.gallery-settings.index',['menu_code' => $pricing->menu_code, 'lang_code' => request('lang_code')]) }}">
            <button type="button" class="btn btn-primary btn-sm m-1">
              <i class="far fa-images me-1"></i> <span> {{__('admin_pricing_page.gallery')}}</span>
            </button>
          </a>
          <!--
            Show on website.
          -->
          <a href="{{ route('slug',$pricing->slug) }}" target="_blank">
            <button type="button" class="btn btn-primary btn-sm m-1">
              <i class="fas fa-external-link-alt me-1"></i> <span> {{__('admin_pricing_page.website')}}</span>
            </button>
          </a>
        </div>
      </div>
  ...
  <!-- Sweet Alert -->
  @if(session('success'))
  <script>swal("{{__('admin_pricing_page.success')}}", "{{__('admin_pricing_page.update_success_context')}}", "success");</script>
  @elseif(session('error'))
  <script>swal("{{__('admin_pricing_page.error')}}", "{{__('admin_pricing_page.update_error_context')}}", "error");</script>
  @elseif(session('file_extension_error'))
  <script>swal("{{__('admin_pricing_page.error')}}", "{{__('admin_pricing_page.update_error_file_mime')}}", "error");</script>
  @elseif(session('file_size_error'))
  <script>swal("{{__('admin_pricing_page.error')}}", "{{__('admin_pricing_page.update_error_file_size')}}", "error");</script>
  @endif
      ...
      <div class="card-body">
        <ul class="nav nav-tabs border-tab" id="top-tab" role="tablist">
          <li class="nav-item"><a class="nav-link @if (!session('tab_page') || session()->get('tab_page') == 'context')) active @endif" data-bs-toggle="tab"
              ...
              <i class="fas fa-book-open"></i>{{__('admin_pricing_page.content')}}</a></li>
          <li class="nav-item"><a class="nav-link @if (session()->get('tab_page') == 'image_large') active @endif" data-bs-toggle="tab"
              ...
              <i class="fas fa-desktop"></i>{{__('admin_pricing_page.big_image')}}</a></li>
          <li class="nav-item"><a class="nav-link @if (session()->get('tab_page') == 'image_small') active @endif" data-bs-toggle="tab"
              ...
              <i class="fas fa-mobile-alt"></i>{{__('admin_pricing_page.small_image')}}</a></li>
          <li class="nav-item"><a class="nav-link @if (session()->get('tab_page') == 'image_banner') active @endif" data-bs-toggle="tab"
              ...
              <i class="fa-solid fa-panorama"></i>{{__('admin_pricing_page.banner_image')}}</a></li>
        </ul>
        <div class="tab-content" id="top-tabContent">
          <div class="tab-pane fade @if (!session()->get('tab_page') || session()->get('tab_page') == 'context')) show active @endif" id="top-content" role="tabpanel" aria-labelledby="top-content-tab">
            <form class="theme-form needs-validation" novalidate="" action="{{ route('admin.pricing-info-settings.update',[$pricing->id]) }}" method="POST">
              @csrf
              @method('PUT')
              <input type="hidden" name="page" value="{{ request('page') }}">
              <h4 class="border-bottom pb-2">{{__('admin_pricing_page.pricingpage_content')}}</h4>
              <div class="mb-3">
                <label class="col-form-label p-0" for="title">{{__('admin_pricing_page.title')}}</label><span class="text-danger">*</span>
                <input class="form-control" id="title" name="title" type="text" placeholder="{{__('admin_pricing_page.title_placeholder')}}" value="{{ $pricing->title }}" required="">
              ...
              <div class="mb-3">
                ...
                <input class="form-control" id="subtitle" name="subtitle" type="text" placeholder="{{__('admin_pricing_page.subtitle_placeholder')}}" value="{{ $pricing->subtitle }}">
              </div>
              <div class="mb-3">
                <label class="col-form-label p-0" for="menu_name">{{__('admin_pricing_page.menu_name')}}</label><span class="text-danger">*</span>
                <input class="form-control" id="menu_name" name="menu_name" type="text" placeholder="{{__('admin_pricing_page.menu_name_placeholder')}}" value="{{ $pricing->menu_name }}" required="">
                <div class="valid-feedback">{{__('admin_pricing_page.looks_good')}}</div>
              </div>
              <div class="mb-3">
                <label class="col-form-label p-0" for="video_embed_code">{{__('admin_pricing_page.youtube_code')}}</label>
                <input class="form-control" id="video_embed_code" name="video_embed_code" type="text" placeholder="{{__('admin_pricing_page.youtube_code_placeholder')}}" value="{{ $pricing->video_embed_code }}">
              </div>
              <div class="mb-3">
                <label class="col-form-label p-0" for="show_button">{{__('admin_pricing_page.show_button')}}</label>
                <input class="form-control" id="show_button" name="show_button" type="text" placeholder="{{__('admin_pricing_page.show_button_placeholder')}}" value="{{ $pricing->show_button }}">
              </div>
              <div class="mb-4">
                <label class="form-label p-0" for="keywords">{{__('admin_pricing_page.keywords')}}</label>
                <textarea class="form-control" id="keywords" name="keywords" rows="4" placeholder="{{__('admin_pricing_page.keywords_placeholder')}}">{{ $pricing->keywords }}</textarea>
              </div>
              <div class="mb-4">
                <label class="form-label p-0" for="description">{{__('admin_pricing_page.description')}}</label>
                <textarea class="form-control" id="description" name="description" rows="8" placeholder="{{__('admin_pricing_page.description_placeholder')}}">{{ $pricing->description }}</textarea>
              </div>
              <div class="mb-4">
                <label class="form-label p-0" for="short_description">{{__('admin_pricing_page.short_description')}}</label>
                <textarea class="form-control" id="short_description" name="short_description" rows="6" placeholder="{{__('admin_pricing_page.short_description_placeholder')}}">{{ $pricing->short_description }}</textarea>
              </div>
              <div>
                <div class="form-check form-switch my-0">
                  <input class="form-check-input custom-switch" type="checkbox" name="is_desktop_active" id="is_desktop_active" @if ($pricing->is_desktop_active) checked @endif>
                  <label class="form-check-label custom-switch-label" for="is_desktop_active">{{__('admin_pricing_page.large_screen_active')}}</label>
                </div>
              </div>
              <div class="mb-4">
                <div class="form-check form-switch my-0">
                  <input class="form-check-input custom-switch" type="checkbox" name="is_mobile_active" id="is_mobile_active" @if ($pricing->is_mobile_active) checked @endif>
                ...
                <div class="row">
                  ...
                  <div class="col-md-6 col-lg-4 col-xl-4 col-xxl-3">
                    <div class="input-group" >
                      <input style="width:100%;" type="submit" class="btn btn btn-primary my-0 mx-1" id="updatepricinginfo" name="updatepricinginfo" value="{{__('admin_pricing_page.update')}}">
                    </div>
                  </div>
                </div>
              </div>
          ...
          <div class="tab-pane fade @if (session()->get('tab_page') == 'image_large') show active @endif" id="top-image-large" role="tabpanel" aria-labelledby="profile-top-tab">
            <form action="{{ route('admin.pricing-info-settings.update',[$pricing->id]) }}" method="POST" enctype="multipart/form-data">
              @csrf
              @method('PUT')
              <input type="hidden" name="page" value="{{ request('page') }}">
              <!-- Image Cropper -->
              <div class="row items-push d-flex align-items-stretch">
                <div class="col-xxl-6">
                  <h4 class="border-bottom pb-2">{{__('admin_pricing_page.large_screen_image')}}</h4>
                  <!--
                  ...
                  <div class="original text-center">
                    ...
                    <img id="js-img-cropper" class="img-fluid" src="{{ asset($pricing->image_large_screen) }}" alt="photo">
                    @else
                  ...
                  <div id="box" class="block-content text-center d-none cropper-buttons">
                    <div class="row">
                      ...
                      data-method="crop" title="{{__('admin_pricing_page.crop_popup')}}">
                      {{__('admin_pricing_page.crop')}}
                      </button>
                    </div>
                  </div>
                  <div class="my-4">
                    <input type="file" class="form-control" id="image" name="image_large_screen" accept="image/*">
                    <small class="fw-light text-gray-dark">{{__('admin_pricing_page.file_mimes')}}</small>
                    <input type="hidden" name="cropped_data" id="cropped_data"> <!--base64 data transferi-->
                  </div>
                </div>

                <div id="box-2" class="col-xxl-6 d-none">
                  <h4 class="border-bottom pb-2">{{__('admin_pricing_page.preview')}}</h4>
                  <!--rightbox-->
                  ...
                  <div class="row">
                    <input type="submit" class="btn btn-lg btn-primary updateimagebutton" id="updateimagelarge" name="updateimagelarge" value="{{__('admin_pricing_page.upload_image')}}">
                  </div>
                </div>
              </div>
              <!-- END Image Cropper -->
          ...
          <div class="tab-pane fade @if (session()->get('tab_page') == 'image_small') show active @endif" id="top-image-small" role="tabpanel" aria-labelledby="contact-top-tab">
            <form action="{{ route('admin.pricing-info-settings.update',[$pricing->id]) }}" method="POST" enctype="multipart/form-data">
              ...
              <div class="row items-push d-flex align-items-stretch">
                <div class="col-xxl-6">
                  <h4 class="border-bottom pb-2">{{__('admin_pricing_page.small_screen_image')}}</h4>
                  <!--
                    RATIO & WIDTH Ayarları.
                  -->
                  ...
                  <div class="original-mobile text-center">
                    @if ($pricing->image_small_screen)
                    <img id="js-img-cropper" class="img-fluid" src="{{ asset($pricing->image_small_screen) }}" alt="photo">
                    @else
                    <img id="js-img-cropper" class="img-fluid" src="{{ asset('uploads/media/constant/placeholder.png') }}" alt="photo">
                    @endif
                  </div>
                ...
                <div id="box-2-mobile" class="col-xxl-6 d-none">
                  ...
                  <div class="row">
                    <input type="submit" class="btn btn-lg btn-primary updateimagebutton" id="updateimagesmall" name="updateimagesmall" value="{{__('admin_pricing_page.upload_image')}}">
                  </div>
                </div>
              </div>
              <!-- END Image Cropper -->
          ...
          <div class="tab-pane fade @if (session()->get('tab_page') == 'image_banner') show active @endif" id="top-image-banner" role="tabpanel" aria-labelledby="top-image-banner-tab">
            <form action="{{ route('admin.pricing-info-settings.update',[$pricing->id]) }}" method="POST" enctype="multipart/form-data">
              ...
              <div class="row items-push d-flex align-items-stretch">
                <div class="col-xxl-6">
                  <h4 class="border-bottom pb-2">{{__('admin_pricing_page.banner_image')}}</h4>
                  <!--
                    RATIO & WIDTH Ayarları.
                  -->
                  ...
                  <div class="original-banner text-center">
                    @if ($pricing->hero)
                    <img id="js-img-cropper" class="img-fluid" src="{{ asset($pricing->hero) }}" alt="photo">
                    @else
                    <img id="js-img-cropper" class="img-fluid" src="{{ asset('uploads/media/constant/placeholder.png') }}" alt="photo">
                    @endif
                  </div>
                ...
                <div id="box-2-banner" class="col-xxl-6 d-none">
                  ...
                  <div class="row">
                    <input type="submit" class="btn btn-lg btn-primary updateimagebutton" id="updateimagebanner" name="updateimagebanner" value="{{__('admin_pricing_page.upload_image')}}">
                  </div>
                </div>
              </div>
              <!-- END Image Cropper -->
